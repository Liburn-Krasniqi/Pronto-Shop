...
const fetchWithRetry = async (url: string, options: RequestInit, retry = true): Promise<any> => {
  try {
...
export const apiClient = {
  ...
  post: async (url: string, body: any) => fetchWithRetry(url, {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
    },
  ...
  put: async (url: string, body: any) => fetchWithRetry(url, {
    method: 'PUT',
    headers: {
      'Content-Type': 'application/json',
    },
